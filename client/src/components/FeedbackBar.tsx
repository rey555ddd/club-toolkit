...
export default function FeedbackBar({ tool, toolContext, outputText, className = "" }: Props) {
  ...
  if (!statusQuery.data?.configured) {
    return (
      <div className={`text-[10px] ${className}`} style={{ color: "rgba(255,255,255,0.3)" }}>
        💡 D1 綁定後可給 AI 回饋
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className={`flex items-center gap-1.5 text-[11px] ${className}`} style={{ color: "rgba(100,200,120,0.9)" }}>
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span className="text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>這版好用嗎？</span>
      <button
        onClick={() => handleClick("up")}
        disabled={submitMutation.isPending}
        className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] transition"
      ...
      <button
        ...
        disabled={submitMutation.isPending}
        className="inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] transition"
        style={{ border: "1px solid rgba(240,192,64,0.35)", color: "rgba(240,192,64,0.9)", background: "rgba(240,192,64,0.06)" }}
